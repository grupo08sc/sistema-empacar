...
const contadorModulo = computed(() => page.props.contadorModulo || 'página actual');
const menu = computed(() => page.props.menu || []);
const empresa = computed(() => page.props.empresa || {});
const fallbackLogo = computed(() => `${page.props.assetUrl}/img/empacar-brand.svg`);
...
const puedeBuscarGlobal = computed(() => {
  ...
  return user.value?.rol?.nombre === 'Administrador' || Boolean(privilegios.Reportes?.leer);
});
...
<template>
  <div class="wrapper">
    <nav class="main-header navbar navbar-expand navbar-white navbar-light">
      <ul class="navbar-nav">
        ...
        <li class="nav-item d-none d-sm-inline-block">
          <Link :href="route(homeRoute)" class="nav-link">Panel principal</Link>
        </li>
      </ul>

      <ul class="navbar-nav ml-auto">
        <li v-if="puedeBuscarGlobal" class="nav-item">
          <a class="nav-link" data-widget="navbar-search" href="#" role="button" aria-label="Buscar información del negocio">
            ...
            <form class="form-inline" @submit.prevent="realizarBusqueda">
              ...
              <div class="input-group input-group-sm">
                ...
                <div class="input-group-append">
                  ...
                  <button class="btn btn-navbar" type="button" data-widget="navbar-search" title="Cerrar búsqueda">
        ...
        <li class="nav-item dropdown">
          ...
          <div class="dropdown-menu dropdown-menu-right">
            <Link :href="route('cargarEstilo', { estilo: '1' })" class="dropdown-item">Tema Niños</Link>
            <Link :href="route('cargarEstilo', { estilo: '2' })" class="dropdown-item">Tema Jóvenes</Link>
            <Link :href="route('cargarEstilo', { estilo: '3' })" class="dropdown-item">Tema Adultos</Link>
            <div class="dropdown-divider"></div>
            <Link :href="route('cargarEstilo', { estilo: '4' })" class="dropdown-item">Modo Día</Link>
            <Link :href="route('cargarEstilo', { estilo: '5' })" class="dropdown-item">Modo Noche</Link>
            <Link :href="route('cargarEstilo', { estilo: '6' })" class="dropdown-item">Automático Día/Noche</Link>
          </div>
        </li>
        ...
        <li class="nav-item dropdown">
          ...
          <div class="dropdown-menu dropdown-menu-right">
            <button class="dropdown-item" type="button" @click="aumentarFuente">Aumentar letra A+</button>
            <button class="dropdown-item" type="button" @click="disminuirFuente">Disminuir letra A-</button>
    ...
    <aside class="main-sidebar sidebar-dark-primary elevation-4">
      <Link :href="route(homeRoute)" class="brand-link d-flex align-items-center" style="text-decoration: none;">
        <img
          :src="logoSrc"
          ...
          @error="onLogoError"
        >
        <span class="brand-text font-weight-bold text-truncate">{{ nombreSistema }}</span>
      </Link>

      <div class="sidebar">
        <nav class="mt-2">
          <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
            <li v-for="grupo in menu" :key="grupo.label" class="nav-item">
              ...
              <ul class="nav nav-treeview">
                <li v-for="item in grupo.items" :key="`${grupo.label}-${item.route}`" class="nav-item">
    ...
    <footer class="main-footer">
      ...
      Sistema comercial y prototipo académico de gestión sostenible.
...
.main-header {
  box-shadow: 0 4px 18px rgba(0, 107, 63, .08) !important;
}

.main-sidebar {
  box-shadow: 8px 0 26px rgba(6, 59, 43, .18) !important;
}
...
.small-box {
  ...
  overflow: hidden;
  box-shadow: 0 12px 30px rgba(6,59,43,.12) !important;
}
...
.small-box.bg-info {
  background: linear-gradient(135deg, var(--empacar-blue), #02A9B5) !important;
}

.small-box.bg-success {
  background: linear-gradient(135deg, var(--empacar-green), var(--empacar-green-light)) !important;
}
...
.small-box.bg-danger {
  background: linear-gradient(135deg, #0B7352, var(--empacar-lime)) !important;
}
