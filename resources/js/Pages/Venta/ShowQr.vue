...
const startPolling = () => {
    ...
    pollInterval = setInterval(async () => {
        ...
        try {
            const response = await fetch(route('pagofacil.estado.cuota', cuotaActual.value.id), {
                headers: {
            ...
            console.log('Estado PagoFácil cuota:', data.estado_pagofacil, data.estado_transaccion);

            if (data.pagado || cuotaActual.value.estado === 'pagado') {
                            ...
                            <div class="card-body p-4 text-center">
                                ...
                                <div v-else>
                                    ...
                                    <div class="border-top pt-3">
                                        ...
                                        <div class="d-flex justify-content-between align-items-center mb-1">
                                            <span class="text-muted small">Monto real de la cuota:</span>
                                            <span class="fw-bold text-muted m-0">{{ formatoMoneda(props.montoRealSistema || cuotaActual.monto) }}</span>
                                        </div>
